...
license: "BSD"
name: "spanner3", "spanner_1x8"
version: "1.0"
Code generated with Faust 2.40.7 (https://faust.grame.fr)
Compilation options: -a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -cn spanner3 -es 1 -mcd 16 -uim -double -ftz 0
------------------------------------------------------------ */

#ifndef  __spanner3_H__
#define  __spanner3_H__
...
class spanner3 : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -cn spanner3 -es 1 -mcd 16 -uim -double -ftz 0");
		m->declare("copyright", "Cecilia-labs");
		m->declare("filename", "spanner3.dsp");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("spanner3");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			output1[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(2.0943951023931953 - fTemp1)))));
			output2[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(4.1887902047863905 - fTemp1)))));
			fRec0[1] = fRec0[0];
			fRec1[1] = fRec1[0];
		}
	}
	...
	#define FAUST_FILE_NAME "spanner3.dsp"
	#define FAUST_CLASS_NAME "spanner3"
	#define FAUST_COMPILATION_OPIONS "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -cn spanner3 -es 1 -mcd 16 -uim -double -ftz 0"
 ...
 */
class spanner3_poly : public dsp_voice_group, public dsp_poly {
    ...
    private:
        ...
        {
            if (val == FAUSTFLOAT(1)) {
                static_cast<spanner3_poly*>(arg)->allNotesOff(true);
            }
        }
    ...
    public:
         ...
         * Constructor.
         *
         * @param dsp - the dsp to be used for one voice. Beware: spanner3_poly will use and finally delete the pointer.
        ...
        virtual spanner3_poly* clone()
        {
            return new spanner3_poly(fDSP->clone(), int(fVoiceTable.size()), fVoiceControl, fGroupControl);
        }
...
struct dsp_poly_factory : public dsp_factory {
    ...
    {
        dsp_poly* dsp_poly = new spanner3_poly(adaptDSP(fProcessFactory->createDSPInstance(), is_double), nvoices, control, group);
        if (fEffectFactory) {
...
void faust_allocate(t_faust* x, int nvoices)
{
    ...
    if (nvoices > 0) {
    ...
    #endif
        x->m_dsp = new spanner3_poly(new spanner3(), nvoices, true, true);
    #ifdef POLY2
        ...
        x->m_dsp = createSRAdapter<FAUSTFLOAT>(new spanner3(), DOWN_SAMPLING, UP_SAMPLING, FILTER_TYPE);
    }
