/* ------------------------------------------------------------
author: "THC-SCALAS"
copyright: "Cecilia-labs"
license: "BSD"
name: "spanner10", "spanner_1x8"
version: "1.0"
Code generated with Faust 2.40.7 (https://faust.grame.fr)
Compilation options: -a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -cn spanner10 -es 1 -mcd 16 -uim -double -ftz 0
------------------------------------------------------------ */

#ifndef  __spanner10_H__
#define  __spanner10_H__
...
class spanner10 : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("author", "THC-SCALAS");
		m->declare("basics_lib_name", "Faust Basic Element Library");
		m->declare("basics_lib_version", "0.6");
		m->declare("compilation_options", "-single -scal -I libraries/ -I project/ -lang wasm");
		m->declare("compile_options", "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -cn spanner10 -es 1 -mcd 16 -uim -double -ftz 0");
		m->declare("copyright", "Cecilia-labs");
		m->declare("filename", "spanner10.dsp");
	...
	virtual void instanceConstants(int sample_rate) {
		fSampleRate = sample_rate;
		fConst0 = std::exp(0.0 - 50.0 / std::min<double>(192000.0, std::max<double>(1.0, double(fSampleRate))));
		fConst1 = 1.0 - fConst0;
	}
	
	virtual void instanceResetUserInterface() {
		fHslider0 = FAUSTFLOAT(12.0);
	...
	virtual spanner10* clone() {
		return new spanner10();
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("spanner10");
		ui_interface->declare(&fEntry0, "style", "knob");
		ui_interface->addNumEntry("Angle", &fEntry0, FAUSTFLOAT(0.0), FAUSTFLOAT(-36000000.0), FAUSTFLOAT(36000000.0), FAUSTFLOAT(0.10000000000000001));
		ui_interface->addHorizontalSlider("Separation", &fHslider0, FAUSTFLOAT(12.0), FAUSTFLOAT(0.0), FAUSTFLOAT(100.0), FAUSTFLOAT(0.01));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		double fSlow1 = fConst1 * double(fEntry0);
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			double fTemp0 = double(input0[i0]);
			fRec0[0] = fSlow0 + fConst0 * fRec0[1];
			fRec1[0] = fSlow1 + fConst0 * fRec1[1];
			double fTemp1 = 0.017453292519943295 * fRec1[0];
			output0[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(-1.0 * fTemp1)))));
			output1[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(0.62831853071795862 - fTemp1)))));
			output2[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(1.2566370614359172 - fTemp1)))));
			output3[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(1.8849555921538759 - fTemp1)))));
			output4[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(2.5132741228718345 - fTemp1)))));
			output5[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(3.1415926535897931 - fTemp1)))));
			output6[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(3.7699111843077517 - fTemp1)))));
			output7[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(4.3982297150257104 - fTemp1)))));
			output8[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(5.026548245743669 - fTemp1)))));
			output9[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(5.6548667764616276 - fTemp1)))));
			fRec0[1] = fRec0[0];
			fRec1[1] = fRec1[0];
		}
	}
	...
	#define FAUST_FILE_NAME "spanner10.dsp"
	#define FAUST_CLASS_NAME "spanner10"
	#define FAUST_COMPILATION_OPIONS "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -cn spanner10 -es 1 -mcd 16 -uim -double -ftz 0"
	#define FAUST_INPUTS 1
	#define FAUST_OUTPUTS 10
	#define FAUST_ACTIVES 2
	#define FAUST_PASSIVES 0

	FAUST_ADDNUMENTRY("Angle", fEntry0, 0.0, -36000000.0, 36000000.0, 0.10000000000000001);
	FAUST_ADDHORIZONTALSLIDER("Separation", fHslider0, 12.0, 0.0, 100.0, 0.01);

	#define FAUST_LIST_ACTIVES(p) \
		p(NUMENTRY, Angle, "Angle", fEntry0, 0.0, -36000000.0, 36000000.0, 0.10000000000000001) \
		p(HORIZONTALSLIDER, Separation, "Separation", fHslider0, 12.0, 0.0, 100.0, 0.01) \
 ...
 */
class spanner10_poly : public dsp_voice_group, public dsp_poly {
    ...
    private:
        ...
        {
            if (val == FAUSTFLOAT(1)) {
                static_cast<spanner10_poly*>(arg)->allNotesOff(true);
            }
        }
    ...
    public:
         ...
         * Constructor.
         *
         * @param dsp - the dsp to be used for one voice. Beware: spanner10_poly will use and finally delete the pointer.
        ...
        virtual spanner10_poly* clone()
        {
            return new spanner10_poly(fDSP->clone(), int(fVoiceTable.size()), fVoiceControl, fGroupControl);
        }
...
struct dsp_poly_factory : public dsp_factory {
    ...
    {
        dsp_poly* dsp_poly = new spanner10_poly(adaptDSP(fProcessFactory->createDSPInstance(), is_double), nvoices, control, group);
        if (fEffectFactory) {
...
void faust_allocate(t_faust* x, int nvoices)
{
    ...
    if (nvoices > 0) {
    ...
    #endif
        x->m_dsp = new spanner10_poly(new spanner10(), nvoices, true, true);
    #ifdef POLY2
        ...
        x->m_dsp = createSRAdapter<FAUSTFLOAT>(new spanner10(), DOWN_SAMPLING, UP_SAMPLING, FILTER_TYPE);
    }
...
void* faust_new(t_symbol* s, short ac, t_atom* av)
{
    ...
    spanner10* tmp_dsp = new spanner10();
