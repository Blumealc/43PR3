...
license: "BSD"
name: "spanner_v", "spanner_1x8"
version: "1.0"
Code generated with Faust 2.40.7 (https://faust.grame.fr)
Compilation options: -a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -cn spanner_v -es 1 -mcd 16 -uim -double -ftz 0
------------------------------------------------------------ */

#ifndef  __spanner_v_H__
#define  __spanner_v_H__
...
class spanner_v : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -cn spanner_v -es 1 -mcd 16 -uim -double -ftz 0");
		m->declare("copyright", "Cecilia-labs");
		m->declare("filename", "spanner_v.dsp");
	...
	virtual spanner_v* clone() {
		return new spanner_v();
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("spanner_v");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			output1[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(1.2566370614359172 - fTemp1)))));
			output2[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(2.5132741228718345 - fTemp1)))));
			output3[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(3.7699111843077517 - fTemp1)))));
			output4[i0] = FAUSTFLOAT(fTemp0 * std::pow(10.0, 0.20000000000000001 * fRec0[0] * (0.0 - 0.5 * (1.0 - std::cos(5.026548245743669 - fTemp1)))));
			fRec0[1] = fRec0[0];
			fRec1[1] = fRec1[0];
		}
	}
	...
	#define FAUST_FILE_NAME "spanner_v.dsp"
	#define FAUST_CLASS_NAME "spanner_v"
	#define FAUST_COMPILATION_OPIONS "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -cn spanner_v -es 1 -mcd 16 -uim -double -ftz 0"
 ...
 */
class spanner_v_poly : public dsp_voice_group, public dsp_poly {
    ...
    private:
        ...
        {
            if (val == FAUSTFLOAT(1)) {
                static_cast<spanner_v_poly*>(arg)->allNotesOff(true);
            }
        }
    ...
    public:
         ...
         * Constructor.
         *
         * @param dsp - the dsp to be used for one voice. Beware: spanner_v_poly will use and finally delete the pointer.
        ...
        virtual spanner_v_poly* clone()
        {
            return new spanner_v_poly(fDSP->clone(), int(fVoiceTable.size()), fVoiceControl, fGroupControl);
        }
...
struct dsp_poly_factory : public dsp_factory {
    ...
    {
        dsp_poly* dsp_poly = new spanner_v_poly(adaptDSP(fProcessFactory->createDSPInstance(), is_double), nvoices, control, group);
        if (fEffectFactory) {
...
void faust_allocate(t_faust* x, int nvoices)
{
    ...
    if (nvoices > 0) {
    ...
    #endif
        x->m_dsp = new spanner_v_poly(new spanner_v(), nvoices, true, true);
    #ifdef POLY2
        ...
        x->m_dsp = createSRAdapter<FAUSTFLOAT>(new spanner_v(), DOWN_SAMPLING, UP_SAMPLING, FILTER_TYPE);
    }
...
void* faust_new(t_symbol* s, short ac, t_atom* av)
{
    ...
    spanner_v* tmp_dsp = new spanner_v();
